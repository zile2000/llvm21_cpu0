                  ...
                  const MCSubtargetInfo &STI) const
{
  ...
  support::endian::write(CB, Binary, Endian);
  // for (unsigned i = 0; i < Size; ++i) {
  //   unsigned Shift = IsLittleEndian ? i * 8 : (Size - 1 - i) * 8;

  //   static const char hex_rep[] = "0123456789abcdef";
  //   unsigned bv = (Binary >> Shift) & 0xff;
  //   LLVM_DEBUG(dbgs()<<hex_rep[(bv & 0xF0) >> 4]<<hex_rep[bv & 0xF]<< " ");
  //   // EmitByte((Val >> Shift) & 0xff, CB);
  //   // support::endian::write<uint8_t>(CB, (Binary >> Shift) & 0xff, llvm::endianness::little);
  // }
  // LLVM_DEBUG(MI.dump();dbgs()<<"\n");
}
