...
bool Cpu0SEDAGToDAGISel::runOnMachineFunction(MachineFunction &MF) {
  LLVM_DEBUG(errs() << "call SEDAGToDAG ";  errs() << "\n");
...
bool Cpu0SEDAGToDAGISel::trySelect(SDNode *Node) {
  LLVM_DEBUG(errs() << "call TrySelect  " ; errs() << "\n");
  unsigned Opcode = Node->getOpcode();
  LLVM_DEBUG(errs() << "opCode  " << Opcode ; errs() << "\n");
  SDLoc DL(Node);
...
void Cpu0SEDAGToDAGISelLegacy::getAnalysisUsage(AnalysisUsage &AU) const{
